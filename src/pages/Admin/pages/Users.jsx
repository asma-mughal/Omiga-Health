...
const Users = () => {
  ...
  const {  addDoctor, 
    editDoctor, 
    deleteDoctor, 
    retrieveDoctor, retrieveHospital,getOneHospital, onHosp,docDeptAdded, setDocDeptAdded} =
  ...
  React.useEffect(()=>{
    retrieveDoctor()
    const x = localStorage.getItem(("DocData"));
    const res = (JSON.parse(x))
    const loopData = (res.data.doctors)
      setData(loopData)
      loopData?.map(mobile => {
  ...
  const handleEditFormSubmit = (event) => {
    ...
    const editedContact = {
      ...
      picture:imageUpload===null? editFormData.picture : imageUpload
    };
...
setDetailHospData(loopData)
 ...
 <form onSubmit={(e)=>e.preventDefault()}>
      ...
      >
        ...
        >
          <div className="py-1">
            {detailHospData?.map((item)=>{
          
              return (  <Menu.Item>
                {({ active }) => (
                  <a
                    onChange={handleAddFormChange}
                    onClick={hospitalHandle(item._id)}
                    className={classNames(
                      active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                      'block px-4 py-2 text-sm'
                    )}
